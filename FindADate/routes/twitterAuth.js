...
//
const oa = new oauth(
    auth.TWITTER.REQUEST_TOKEN_URL,
    auth.TWITTER.ACCESS_TOKEN_URL,
    auth.TWITTER.CONSUMER_KEY,
    auth.TWITTER.CONSUMER_SECRET,
    auth.TWITTER.OAUTH_VERSION,
    auth.TWITTER.CALLBACK_URL,
    auth.TWITTER.ALGORITHM
);
...
router.get('/showAccount', checkAuthorization, function (req, res, next) {
    ...
    client.get(auth.TWITTER.urls.ACCOUNT_SETTINGS_URL, function (err, account, response) {
